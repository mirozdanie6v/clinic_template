...
for (const branch of ["center", "north", "saigon"]) {
  ...
  for (const name of fs.readdirSync(dir).filter((n) => n.endsWith(".json")).sort()) {
    ...
    if (locationId) payloads.push({ locationId, payload });
  }
}
if (!payloads.length) throw new Error("exact-network-v2: no snapshots found");

const categories = new Map();
const services = new Map();
const staff = new Map();
for (const { locationId, payload } of payloads) {
  for (const c of payload.categories || []) categories.set(`${locationId}:${c.id}`, { ...c, locationId });
  for (const s of payload.services || []) services.set(`${locationId}:${s.id}`, { ...s, locationId });
  for (const p of payload.staff || []) staff.set(`${locationId}:${p.id}`, { ...p, locationId });
}
...
for (const s of services.values()) {
  ...
  if (!groups.has(groupId)) {
    ...
    groups.set(groupId, { id: groupId, title: localize(`${categoryName} · ${branchName}`), note: localize(note), image: passport.brand?.images?.hero || "/client/hero.webp", items: [] });
  }
  const minutes = Number(s.durationMinutes || 0);
  const staffIds = externalStaffIds.map((id) => `altegio-staff-${s.locationId}-${id}`);
  groups.get(groupId).items.push({ id: serviceId, name: localize(s.name, serviceId), price: localize(money(s)), desc: minutes ? { ru: `${minutes} мин`, en: `${minutes} min`, vi: `${minutes} phút` } : localize(""), image: s.image || passport.brand?.images?.hero || "/client/hero.webp", staffIds, locationId: `altegio-${s.locationId}`, externalId: s.id });
...
for (const p of staff.values()) {
  ...
  allStaff.push({ id: `altegio-staff-${p.locationId}-${p.id}`, name: localize(p.name, String(p.id)), role: localize(p.specialization || "Specialist"), image: p.avatar || p.avatarSmall || passport.brand?.images?.hero || "/client/hero.webp", tags: [], serviceGroups, serviceIds, locationId: `altegio-${p.locationId}`, externalId: p.id, bookable: p.bookable !== false, scheduleTill: p.scheduleTill || null });
}
...
console.log(`exact-network-v2: ${finalGroups.length} groups, ${allServices.length} services, ${allStaff.length} staff, ${links} relations, ${selectable.length} selectable specialists`);
